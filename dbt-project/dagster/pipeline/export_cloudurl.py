        ...
        while i < dataset_pages:
            ...
            if dq_datasets.status_code == 200:
                ...
                for dataset in page_data:
                    ...
                    if (
                        dataset_name in input_table_names
                        and datasource_name in input_datasources
                    ):
                ...
                print(
                    f"API Rate Limit reached when fetching datasets on page: {i}. Pausing for 30 seconds."
                )
                time.sleep(30)
            else:
                print(
                    f"Error fetching datasets on page {i}. Status code: {dq_datasets.status_code}"
                )
                sys.exit()
    else:
        print(
            f"Error fetching initial datasets. Status code: {response_datasets.status_code}"
        )
        sys.exit()
